    ...
    def load_rgb(self, file_path):
        ...
        if not self.lmdb:
            if not os.path.isabs(file_path):
                # Usamos os.path.join para compatibilidade (ele usará / no Colab)
            ...
            with self.env.begin(write=False) as txn:
                ...
                if image_bin is None:
                  raise ValueError(f"Chave LMDB não encontrada: {file_path}. Verifique se o JSON e o LMDB estão sincronizados.")
